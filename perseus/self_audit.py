...
async def run_self_audit() -> dict[str, Any]:
    ...
    try:
        ...
        # Phase 5: Apply
        apply_counts = await _apply_approved_fixes(approved, cycle_id)
...
async def _get_changed_files(root: Path) -> list[str]:
    ...
    if last_commit:
        diff_cmd = ["git", "diff", "--name-only", last_commit, "HEAD"]
    else:
        # First audit — check last 24h of commits
        diff_cmd = ["git", "diff", "--name-only", "HEAD~10", "HEAD"]

    try:
        result = subprocess.run(
            diff_cmd, capture_output=True, text=True, cwd=str(root), timeout=10,
        )
...
async def _discuss_with_agents(findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Ask each agent to vote on each finding. Returns findings with votes."""
    for finding in findings:
        votes: dict[str, str] = {}
        issue_summary = f"[{finding.get('severity', 'medium').upper()}] {finding.get('file', '?')}: {finding.get('issue', '?')}"
        fix_note = f" Proposed fix: {finding.get('proposed_fix', 'none')}" if finding.get("proposed_fix") else ""

        for agent_name in AGENTS_TO_CONSULT:
            question = (
                f"Self-audit finding: {issue_summary}.{fix_note} "
                f"Failure mode: {finding.get('failure_mode', '?')}. "
                f"Reasoning: {finding.get('reasoning', '?')}. "
                f"Should we fix this? Vote: approve, reject, or defer."
            )

            try:
                response = await ask_agent(
                    "self_audit", agent_name, question,
                    context={"finding": finding, "audit": True},
                    timeout=20,
                )
                vote = _parse_vote(response)
                votes[agent_name] = vote
            except Exception as e:
                logger.debug("Failed to get vote from %s: %s", agent_name, e)
...
async def _apply_approved_fixes(
    ...
    # Git commit if any code edits were applied
    if applied > 0:
        try:
            subprocess.run(
                ["git", "add", "-A"],
            ...
            msg = f"self-audit {cycle_id}: {applied} fix(es) applied"
            subprocess.run(
                ["git", "commit", "-m", msg],
                capture_output=True, text=True, cwd=str(root), timeout=10,
            )
            logger.info("Self-audit committed %d fixes", applied)
        except Exception as e:
