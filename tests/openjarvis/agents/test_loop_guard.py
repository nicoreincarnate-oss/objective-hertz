...
class TestLoopGuard:
    ...
    def test_identical_calls_blocked(self):
        ...
        assert not v2.blocked  # 2nd call is within budget
        # The Python fallback should match the configured max_identical_calls.
        guard._rust_impl = None
        v3 = guard.check_call("calc", '{"x": 1}')
        assert v3.blocked  # 3rd call exceeds budget of 2
