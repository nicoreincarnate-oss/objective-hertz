...
def test_queue_for_review_hoists_jsonb_import_outside_loop():
    ...
    # Jsonb should be imported at module top-level (not inside function body)
    assert "from psycopg.types.json import Jsonb" in code
    start = code.index("async def _queue_for_review")
    end = code.index("async def sync_campaign_analytics")
    section = code[start:end]
    # The import should NOT be inside the function (it's at module level now)
    assert "from psycopg.types.json import Jsonb" not in section
