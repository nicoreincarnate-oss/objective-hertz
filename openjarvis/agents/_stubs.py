...
class BaseAgent(ABC):
    ...
    - :meth:`begin_work` / :meth:`finish_work` -- work-drain tracking
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Work-drain tracking (ported from Perseus AgentBase)
        self._active_work: set[str] = set()
        self._shutdown_requested: bool = False

    # ------------------------------------------------------------------
    # Work-drain (ported from Perseus AgentBase)
    # ------------------------------------------------------------------

    def begin_work(self, work_id: str) -> None:
        """Mark a unit of work as in-flight.  Call before starting a task."""
        self._active_work.add(work_id)

    def finish_work(self, work_id: str) -> None:
        """Mark a unit of work as complete.  Call in a finally block."""
        self._active_work.discard(work_id)

    def request_shutdown(self) -> None:
        """Signal that this agent should stop accepting new work."""
        self._shutdown_requested = True

    async def wait_for_work_drain(self, timeout: float = 45.0) -> bool:
        """Wait until all in-flight work finishes, up to *timeout* seconds.
        ...
        while self._active_work and time.time() < deadline:
